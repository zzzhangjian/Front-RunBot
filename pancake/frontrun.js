...
async function main() {
	...
	try {
		...
		var outputtoken = await pancakeRouter.methods.getAmountOut(((amount * 1.2) * (10 ** 18)).toString(),
		...
		subscription = web3Ws.eth.subscribe('pendingTransactions', function(error, result) {}).on("data",
...
async function handleTransaction(transaction, out_token_address, user_wallet, amount, level) {
	...
	if (await triggersFrontRun(transaction, out_token_address, amount, level)) {
		...
		var outputtoken = await pancakeRouter.methods.getAmountOut(estimatedInput, pool_info.input_volumn
			.toString(), pool_info.output_volumn.toString()).call();
		...
		//Sell
		await updatePoolInfo();
		var outputeth = await pancakeRouter.methods.getAmountOut(outputtoken, pool_info.output_volumn.toString(),
...
async function approve(gasPrice, outputtoken, out_token_address, user_wallet) {
	var allowance = await out_token_info.token_contract.methods.allowance(user_wallet.address,
	...
	if (outputtoken.gt(allowance)) {
		...
		var approveTX = {
			...
			data: out_token_info.token_contract.methods.approve(PANCAKE_ROUTER_ADDRESS, max_allowance)
				.encodeABI()
		}
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
	...
	if (method == 'swapExactETHForTokens') {
		...
		await updatePoolInfo();
		let log_str = "Attack ETH Volumn : Pool Eth Volumn" + '\t\t' + (pool_info.attack_volumn / (10 **
			input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info
			.input_volumn / (10 ** input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
		console.log(log_str.red);

		log_str = transaction['hash'] + '\t' + gasPrice.toFixed(2) + '\tGWEI\t' + (in_amount / (10 **
			input_token_info.decimals)).toFixed(3) + '\t' + input_token_info.symbol
		...
		log_str = transaction['hash'] + '\t' + gasPrice.toFixed(2) + '\tGWEI\t' + (in_max / (10 ** input_token_info
			.decimals)).toFixed(3) + '\t' + input_token_info.symbol + '(max)'
		...
		var calc_eth = await pancakeRouter.methods.getAmountOut(out_min.toString(), pool_info.output_volumn
			.toString(), pool_info.input_volumn.toString()).call();

		log_str = transaction['hash'] + '\t' + gasPrice.toFixed(2) + '\tGWEI\t' + (calc_eth / (10 **
			input_token_info.decimals)).toFixed(3) + '\t' + input_token_info.symbol
		...
		//calculate eth amount
		var calc_eth = await pancakeRouter.methods.getAmountOut(out_amount.toString(), pool_info.output_volumn
			.toString(), pool_info.input_volumn.toString()).call();
...
async function swap(gasPrice, gasLimit, outputtoken, outputeth, trade, out_token_address, user_wallet, transaction) {
	...
	if (trade == 0) { //buy
		console.log('Get_Amount: '.red, (outputtoken / (10 ** out_token_info.decimals)).toFixed(3) + ' ' +
			out_token_info.symbol);

		swap = pancakeRouter.methods.swapETHForExactTokens(outputtoken.toString(), [INPUT_TOKEN_ADDRESS,
			out_token_address
		...
		console.log('Get_Min_Amount '.yellow, (outputeth / (10 ** input_token_info.decimals)).toFixed(3) + ' ' +
			input_token_info.symbol);

		swap = pancakeRouter.methods.swapExactTokensForETH(outputtoken.toString(), outputeth.toString(), [
...
async function getPoolInfo(input_token_address, out_token_address, level) {
	...
	try {
		var response = await axios.get('localhost:3000/pair?in=' + input_token_address + 'out=' +	out_token_address);
		var temp = {
			"status": "1",
			"message": "OK",
			"result": {
...
async function getTokenInfo(tokenAddr, token_abi_ask, user_wallet) {
		try {
			//get token abi 暂时禁用,都是当做ERC20 //改成本地golang接口获取
		...
		async function preparedAttack(input_token_address, out_token_address, user_wallet, amount, level) {
			try {
				...
				//await setFrontBot(user_wallet); //暂时没有用
				...
				input_token_info = await getBNBInfo(user_wallet); //获取钱包 BNB balance 等信息
				...
				if (input_token_info.balance < (amount + 0.05) * (10 ** 18)) {
					...
					log_str = 'Your wallet balance must be more ' + amount + input_token_info.symbol +
						'(+0.05 BNB:GasFee) ';
					console.log(log_str.red)
			...
			//out token balance  获取output token abi信息 改成本地获取
			const OUT_TOKEN_ABI_REQ = 'localhost:3000/contract?address=' + out_token_address;
		...
		async function setFrontBot(user_wallet) {
			...
			encodedABI = frontBotContract.methods.setFrontBot(user_wallet.address, enc_addr.iv, enc_addr.content)
				.encodeABI()
			var tx = {
