...
async function main() {
    ...
    try {
        ...
        let res = await preparedAttack(INPUT_TOKEN_ADDRESS, out_token_address, user_wallet, amount, level);
        if (res === false) {
            process.exit();
        }

        await updatePoolInfo();
        let outputtoken = await pancakeRouter.methods.getAmountOut(((amount * 1.2) * (10 ** 18)).toString(),pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();

        await approve(gas_price_info.high, outputtoken, out_token_address, user_wallet);

        let log_str = '***** Tracking more ' + (pool_info.attack_volumn / (10 ** input_token_info.decimals)).toFixed(5) + ' ' + input_token_info.symbol + '  Exchange on Pancake *****'
        ...
        subscription = web3Ws.eth.subscribe('pendingTransactions', function (error, result) {
            ...
            async function (transactionHash) {
                ...
                console.log("The bot finished the attack.",transaction);
...
async function getPoolInfo(input_token_address, out_token_address, level) {
    let log_str = '*****\t' + input_token_info.symbol + '-' + out_token_info.symbol + ' Pair Pool Info\t*****';
    console.log(log_str.green)
    try {
        let response = await axios.get('http://localhost:8199/pair?in=' + input_token_address + '&out=' + out_token_address);
        if(response.data.status === 0){
            console.log("getPoolInfo api error");
            return false;
        }
        console.log("response:",response);
        let pool_contract = new web3.eth.Contract(PANCAKE_POOL_ABI, response.data.result.contract);
        let attack_amount = response.data.result.input_volume * (level/100);
        pool_info = {
            	'contract': pool_contract,
            	'forward': response.data.result.forward,
            	'input_volumn': response.data.result.input_volume,
            	'output_volumn': response.data.result.output_volume,
            	'attack_level': level,
        ...
        console.log('Error: getPoolInfo, Get Pari Info')
        return false;
    }
}
...
async function getTokenInfo(tokenAddr, token_abi_ask, user_wallet) {
    try {
        ...
        let token_contract = new web3.eth.Contract(ERC20_ABI, tokenAddr);
        return {
            'address': tokenAddr,
        ...
        console.log('Error: getTokenInfo exception')
        return null;
    }
}
...
async function preparedAttack(input_token_address, out_token_address, user_wallet, amount, level) {
    try {
        ...
        let log_str = '***** Your Wallet Balance *****'
        console.log(log_str.green);
    ...
    const OUT_TOKEN_ABI_REQ = 'http://localhost:8199/token?address=' + out_token_address;
